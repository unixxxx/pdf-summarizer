...
class PDFService:
    ...
    async def extract_metadata(self, pdf_content: bytes, filename: str) -> PDFMetadata:
        """
        ...
        try:
            ...
            if reader.metadata:
                ...
                metadata_dict.update(
                    {
                        "title": self._clean_metadata_value(
                            reader.metadata.get("/Title")
                        ),
                        "author": self._clean_metadata_value(
                            reader.metadata.get("/Author")
                        ),
                        "subject": self._clean_metadata_value(
                            reader.metadata.get("/Subject")
                        ),
                        "creator": self._clean_metadata_value(
                            reader.metadata.get("/Creator")
                        ),
                    }
                )
