...
class ArchiveService:
    ...
    async def restore_documents(
        self, db: AsyncSession, user: User, document_ids: list[UUID]
    ) -> None:
        """Restore multiple documents from archive."""
        for document_id in document_ids:
            await self.restore_document(db, user, document_id)

    async def permanently_delete_documents(
        self, db: AsyncSession, user: User, document_ids: list[UUID]
    ) -> None:
        """Permanently delete multiple documents from archive."""
        for document_id in document_ids:
            await self.permanently_delete_document(db, user, document_id)
    ...
    async def empty_archive(self, db: AsyncSession, user: User) -> dict:
        ...
        # Collect paths from standalone documents
        for doc in standalone_docs:
            if doc.storage_path:
                storage_paths_to_delete.append(doc.storage_path)
        
        # Collect paths from documents in folders
        for folder in folders:
            for doc in folder.documents:
        ...
        if self.storage_service and storage_paths_to_delete:
            await self.storage_service.delete_files_batch(storage_paths_to_delete)
        
        # Bulk delete all documents first (to avoid foreign key constraints)
        if standalone_docs:
            # Get all document IDs for bulk deletion
            doc_ids = [doc.id for doc in standalone_docs]
            await db.execute(
                delete(Document).where(Document.id.in_(doc_ids))
            )
        
        # Delete all documents in folders before deleting the folders themselves
        if folders:
            # First, delete all documents that belong to these folders
