...
from ..database.models import Document, Tag, document_tags
from .schemas import TagGenerationRequestInternal
...
class TagService:
    ...
    async def generate_tags_for_document(
        self,
        request: TagGenerationRequestInternal,
    ) -> list[dict[str, Any]]:
...
"""
    ...
    async def get_all_tags_with_counts(
        ...
        result = await db.execute(
            ...
            .where(Document.deleted_at.is_(None))  # Exclude trashed documents
            .group_by(Tag.id)
    ...
    async def generate_tags_for_summary(
        self,
        summary_id: UUID,
        user_id: UUID,
        db: AsyncSession,
        ...
        if not document:
            raise ValueError(f"Document {summary_id} not found for user {user_id}")
        
        # Create request for tag generation
        request = TagGenerationRequestInternal(
        ...
        # Generate tags
        tag_data_list = await self.generate_tags_for_document(request)
        
        # Find or create tags in database
        tags = await self.find_or_create_tags(tag_data_list, db)
        
        # Associate tags with document
        await self.associate_tags_with_document(
            document.id,
    ...
    async def update_tag(
        ...
        tag.name = name
        tag.slug = self._create_slug(name)
