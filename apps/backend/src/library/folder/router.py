...
async def get_folders_tree(
    ...
    folders = await service.get_folders_tree(db, current_user)
    unfiled_count = await service.get_unfiled_document_count(db, current_user)
    total_document_count = await service.get_total_document_count(db, current_user)
    return FolderTree(
        folders=folders, 
