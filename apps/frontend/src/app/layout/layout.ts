...
@Component({
  selector: 'app-layout',
  standalone: true,
  imports: [RouterModule, RouterOutlet, Notifications, GlobalLoading],
  ...
  template: `
    <div
      class="h-screen flex flex-col bg-background transition-theme gradient-mesh overflow-hidden"
    >
      <nav class="glass flex-shrink-0 z-50 border-b border-border/50">
        ...
        <!-- Mobile menu -->
        @if (isMobileMenuOpen()) {
        <div class="sm:hidden border-t border-border/50" [@slideInOut]>
...
export class Layout {
  ...
  readonly currentUser = toSignal(
    this.store.select(authFeature.selectUser).pipe(map(unwrapAsyncDataItem))
  );
  ...
  logout() {
    this.store.dispatch(AuthActions.logoutCommand());
  }
