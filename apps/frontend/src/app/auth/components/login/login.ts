...
import { queryAsyncState } from '../../../core/utils/async-data-item';
...
export class Login {
  ...
  readonly isLoading = toSignal(
    this.store
      ...
      .pipe(map((user) => queryAsyncState(user).isLoading))
  );

  readonly isDarkMode = this.themeStore.isDarkMode;

  login(provider: 'google' | 'github') {
    this.store.dispatch(AuthActions.loginCommand({ provider }));
  }
