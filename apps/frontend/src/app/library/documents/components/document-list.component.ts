...
@Component({
  ...
  template: `
    ...
    <div class="p-6">
      @if (libraryStore.documentsLoading()) {
      ...
      } @else if (libraryStore.documentsIsEmpty() && !libraryStore.documentsIsSearching()) {
        <div class="text-center py-12">
          ...
          <p class="mt-1 text-sm text-muted-foreground">
            @if (libraryStore.selectedFolderId() === 'unfiled') {
              No unfiled documents. Great job organizing!
            } @else if (libraryStore.selectedFolderId()) {
              This folder is empty. Drag documents here to organize them.
            } @else {
...
export class DocumentListComponent {
  ...
  exportDocument(event: { item: LibraryItem; format: 'pdf' | 'markdown' | 'text' }) {
