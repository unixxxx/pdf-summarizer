...
@Component({
  ...
  template: `
    @if (libraryStore.showDeleteDocumentConfirm() &&
    libraryStore.deletingDocument()) {
    <!-- Modal Backdrop -->
    ...
    >
      ...
      >
        ...
        <div class="px-6 py-4 border-b border-border">
          <h3 class="text-lg font-semibold text-foreground">Move to Archive</h3>
        </div>
        ...
        <div class="px-6 py-4">
          <p class="text-foreground">
            ...
            <strong class="font-semibold"
              >"{{ libraryStore.deletingDocument()?.filename }}"</strong
            >
            to archive?
          </p>
          ...
          <div class="mt-3 p-3 bg-muted/50 border border-muted rounded-lg">
            <p class="text-sm text-muted-foreground">
              ...
              The document will be moved to archive where you can restore it
              later if needed.
            </p>
          </div>

          @if (libraryStore.deletingDocument()?.summary?.tags &&
          libraryStore.deletingDocument()!.summary.tags.length > 0) {
          <div class="mt-3">
            <p class="text-sm text-muted-foreground">
              This document has
              {{ libraryStore.deletingDocument()!.summary.tags.length }} tag(s)
              associated with it.
            </p>
          </div>
          }
