  ...
  template: `
    ...
    <div 
      ...
      <div class="relative group/folder">
        ...
        >
          <svg
            [@chevronRotate]="expandedFolders().includes(folder.id) ? 'expanded' : 'collapsed'"
        ...
        <div
          ...
          (keydown.enter)="folderSelected.emit(folder.id)"
          (keydown.space)="folderSelected.emit(folder.id)"
          [class.bg-muted]="selectedFolderId() === folder.id"
          [class.text-primary-600]="selectedFolderId() === folder.id"
        ...
        >
        <button
          (click)="folderEdit.emit({ folder: folder, event: $event }); $event.stopPropagation()"
        ...
        <button
          (click)="folderDelete.emit({ folder: folder, event: $event }); $event.stopPropagation()"
          (keydown.enter)="folderDelete.emit({ folder: folder, event: $event })"
          (keydown.space)="folderDelete.emit({ folder: folder, event: $event })"
      ...
      <!-- Nested Folders -->
      @if (folder.children && folder.children.length > 0 && expandedFolders().includes(folder.id)) {
      <div 
        @expandCollapse
        class="ml-4 origin-top"
      >
        <app-folder-tree
          @fadeIn
...
export class FolderTreeComponent {
  // Input signals
  folders = input<FolderWithChildren[]>([]);
