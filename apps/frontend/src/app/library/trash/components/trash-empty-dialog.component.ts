...
@Component({
  selector: 'app-trash-empty-dialog',
  standalone: true,
      ...
      >
        ...
        >
          ...
          <div class="px-6 py-4 border-b border-border">
            <h3 class="text-lg font-semibold text-foreground">Empty Trash</h3>
          </div>
          ...
          <div class="px-6 py-4">
              ...
              <div class="flex items-start">
                ...
                <div class="text-sm">
                  ...
                  <p class="text-error/90">
                    All items in trash will be permanently deleted and cannot be recovered.
                  </p>
                </div>
              </div>
            </div>
            ...
            @if (documentCount() > 0 || folderCount() > 0) {
              <div class="mt-4 p-3 bg-muted/50 border border-muted rounded-lg text-sm">
                <p class="font-medium text-foreground mb-2">You are about to permanently delete:</p>
                <ul class="space-y-1 text-muted-foreground">
                  @if (documentCount() > 0) {
                    <li>• {{ documentCount() }} document{{ documentCount() > 1 ? 's' : '' }}</li>
                  }
                  @if (folderCount() > 0) {
                    <li>• {{ folderCount() }} folder{{ folderCount() > 1 ? 's' : '' }}</li>
                  }
                  @if (totalSize() > 0) {
...
export class TrashEmptyDialogComponent {
