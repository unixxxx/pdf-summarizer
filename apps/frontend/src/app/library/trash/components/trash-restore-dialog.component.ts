...
@Component({
  selector: 'app-trash-restore-dialog',
  standalone: true,
          ...
          <div class="px-6 py-4">
            ...
            <div class="mt-3 p-3 bg-muted/50 border border-muted rounded-lg">
              <p class="text-sm text-muted-foreground">
                ...
                @if (isFolder()) {
                  This folder and all its contents will be restored to 
                  {{ parentName() ? parentName() : 'the root level' }}.
                } @else {
            ...
            @if (isFolder() && (documentCount() > 0 || childrenCount() > 0)) {
              <div class="mt-3 text-sm text-muted-foreground">
                ...
                <ul class="space-y-1 ml-4">
                  ...
                  @if (childrenCount() > 0) {
                    <li>• {{ childrenCount() }} subfolder{{ childrenCount() > 1 ? 's' : '' }}</li>
                  }
                </ul>
              </div>
            }
...
export class TrashRestoreDialogComponent {
